...
class ECDSA2():
    ...
    def KeyGen(self) -> Tuple[IntegerMod_int, Point]:
        ...
        X = x * self.P
        # print("the value of x is ", x, " and the X is ", X)
        return (x, X)
    ...
    def Sign_FixedNonce(self, nonce: IntegerMod_int, privkey: IntegerMod_int, msg: str) -> Tuple[IntegerMod_int, IntegerMod_int]:
        ...
        nonce = self.Z_q(nonce)
        s = self.Z_q(1/nonce * (h**2 + constant * privkey * r))
        return (r, s)
    ...
    def Sign(self, privkey, msg) -> Tuple[IntegerMod_int, IntegerMod_int]:
        ...
        r, s = 0, 0
        while r == 0 or s == 0:
            nonce = self.Z_q(randint(1, self.q - 1))
            r, s = self.Sign_FixedNonce(nonce, privkey, msg)
        return r, s
    ...
    def Verify(self, pubkey: Point, msg: str, r: IntegerMod_int, s: IntegerMod_int) -> bool:
        ...
        if r <= 1 or r >= self.q-1:
            raise ValueError("r out of range")
        if s <= 1 or s >= self.q-1:
        ...
        return 1 if self.Z_q(Z.x) == r else 0
